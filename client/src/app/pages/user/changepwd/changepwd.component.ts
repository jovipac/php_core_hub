  ...
  cambiarClave(event: Event) {
    ...
    if (this.frmCambiarClave.valid) {
      ...
      if (this.frmCambiarClave.value['nuevaClave'] == this.frmCambiarClave.value['confNuevaClave']) {
        ...
        let objectToSend = {
          nuevaClave: this.frmCambiarClave.value['confNuevaClave'],
          id_funcionario: codesSession['id_usuario']
        };

        ////se ejecuta el servicio
        this.service.updatePassword(codesSession['id_usuario'], objectToSend).subscribe(res => {
