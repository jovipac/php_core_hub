...
trait Userstamps
{
    ...
    {
        static::creating('App\Http\Traits\Listeners\Creating@handle');
        static::updating('App\Http\Traits\Listeners\Updating@handle');

        if (static::usingSoftDeletes()) {
            static::deleting('App\Http\Traits\Listeners\Deleting@handle');
            static::restoring('App\Http\Traits\Listeners\Restoring@handle');
        }
    }
    ...
    public static function usingSoftDeletes()
    {
        ...
        if (is_null($usingSoftDeletes)) {
            return $usingSoftDeletes = in_array('App\Http\Traits\SoftDeletesBoolean', class_uses_recursive(get_called_class()));
        }
